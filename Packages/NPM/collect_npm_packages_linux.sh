...
# ==============================================================================
# SCRIPT: collect_npm_packages_linux.sh
# DESCRIPTION: Collect installed npm packages on Linux.
...
#
# SCANNED LOCATIONS:
#   - System-wide: /usr/local/lib/node_modules, /usr/lib/node_modules
#   - Per-user npm prefix: ~/.npm-global/lib/node_modules, ~/npm-global/lib/node_modules
#   - Per-user nvm: ~/.nvm/versions/node/<version>/lib/node_modules
#   - Per-user fnm: ~/.local/share/fnm/node-versions/<v>/installation/lib/node_modules
#   - Per-user Volta: ~/.volta/tools/image/packages
#   - Per-user n: ~/n/lib/node_modules (tj/n version manager)
#
# BUILT-IN TOOLS ONLY (no Python or jq required):
#   bash, find, stat, date, hostname, sed, tr, grep, uname
# ==============================================================================
...
function get_author_string {
    ...
    local content
    content=$(tr -d '\n' < "$json_file" 2>/dev/null)
    author=$(echo "$content" | sed -n 's/.*"author"[[:space:]]*:[[:space:]]*{[^}]*"name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -1)
    echo "$author"
}
...
function get_os_info_json {
    ...
    local os_id os_version os_build
    ...
    os_build=$(uname -r 2>/dev/null)
    local collection_date
    collection_date=$(date "+%Y-%m-%d %H:%M:%S")

    echo "{\"RecordType\":\"EndpointInfo\",\"EndpointHostname\":\"$(json_escape "$hostname")\",\"EndpointOS\":\"Linux\",\"EndpointOSVersion\":\"$(json_escape "${os_id} ${os_version}")\",\"EndpointOSBuild\":\"$(json_escape "$os_build")\",\"EndpointCollectionDate\":\"$collection_date\"}"
}

function get_creation_timestamp {
    local path="$1"
    # Linux stat: %W = birth time (0 if unavailable), %Y = modification time
    local birth
    birth=$(stat -c "%W" "$path" 2>/dev/null)
    if [[ -n "$birth" && "$birth" != "0" && "$birth" != "-" ]]; then
        date -d "@$birth" "+%Y-%m-%d %H:%M:%S" 2>/dev/null
    else
        stat -c "%y" "$path" 2>/dev/null | sed 's/\([0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\) \([0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}\).*/\1 \2/'
    fi
}
...
function collect_user_dirs {
    ...
    # Also include root's home if running as root
...
function scan_user_packages {
    ...
    for user_dir in "${user_dirs[@]}"; do
        ...
        local npm_paths=(
            ...
            "$user_dir/.local/lib/node_modules"
        )
        ...
        # 3. fnm: ~/.local/share/fnm/node-versions/<v>/installation/lib/node_modules
        local fnm_root="$user_dir/.local/share/fnm/node-versions"
        if [[ -d "$fnm_root" ]]; then
            local version_dir
        ...
        # 5. n (tj/n): ~/n/lib/node_modules
        local n_root="$user_dir/n/lib/node_modules"
        if [[ -d "$n_root" ]]; then
            write_log "Scanning n version manager for user '$user_name': $n_root"
            scan_node_modules "$n_root" "UserGlobal" "$user_name" "n"
        fi
    done
}
...
if test_is_remote_ops; then
    ...
    OUTPUT_FILE="/tmp/NPM_Packages_Inventory_${timestamp}.json"
    LOG_FILE="/tmp/NPM_Packages_Inventory_${timestamp}.log"
else
    OUTPUT_FILE="/tmp/NPM_Packages_Inventory.json"
    LOG_FILE="/tmp/NPM_Packages_Inventory.log"
fi
