...
# ==============================================================================
# SCRIPT: collect_cursor_extensions.sh
# DESCRIPTION: Collect installed Cursor extensions on macOS.
# AUTHOR: Jean-Marc ALBERT
# DATE: 2026-03-12
# VERSION: 1.0
#
# OBJECTIVE:
#   Inventory Cursor extensions from user profiles and export a JSON file that
...
function json_extract_raw {
    ...
    if [[ -z "$value" ]] && [[ -f "$json_file" ]]; then
        # Fallback: sed-based extraction (built-in macOS tools only)
        local key
        if [[ "$key_path" == *.* ]]; then
            # Nested key (e.g. engines.vscode): match parent.key within JSON
...
function json_extract_array {
    ...
    if [[ -z "$value" || ! "$value" =~ ^\[.*\]$ ]] && [[ -f "$json_file" ]]; then
        # Fallback: sed-based extraction for arrays (built-in macOS tools only)
        local key="${key_path##*.}"
        local content
        content=$(tr -d '\n' < "$json_file" 2>/dev/null)
...
function collect_cursor_extensions_for_product {
    ...
    for user_dir in "${user_dirs[@]}"; do
        ...
        while IFS= read -r ext_dir; do
            ...
            record+="\"ExtensionPlatform\":\"Cursor\","
...
if test_is_remote_ops; then
    OUTPUT_FILE="$S1_OUTPUT_DIR_PATH/Cursor_Extensions_Inventory.json"
    LOG_FILE="$S1_OUTPUT_DIR_PATH/Cursor_Extensions_Inventory.log"
else
    OUTPUT_FILE="${TMPDIR:-/tmp}/Cursor_Extensions_Inventory_${timestamp}.json"
    LOG_FILE="${TMPDIR:-/tmp}/Cursor_Extensions_Inventory_${timestamp}.log"
fi
...
write_log "Starting Cursor extension inventory..."
...
# Cursor may store extensions in either location depending on version/config:
# - ~/Library/Application Support/Cursor/User/extensions (standard Electron app path)
# - ~/.cursor/extensions (alternative path used by some Cursor versions)
collect_cursor_extensions_for_product "Cursor" "Library/Application Support/Cursor/User/extensions" "${USER_DIRS[@]}"
collect_cursor_extensions_for_product "Cursor" ".cursor/extensions" "${USER_DIRS[@]}"
...
if [[ $extension_count -gt 0 ]]; then
    ...
    write_log "No Cursor extensions found. Metadata-only JSON saved at: $OUTPUT_FILE" "INFO"
fi
