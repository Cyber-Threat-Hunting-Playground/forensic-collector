...
# ==============================================================================
# SCRIPT: collect_vscodium_extensions.sh
# DESCRIPTION: Collect installed VSCodium extensions on macOS.
# AUTHOR: Jean-Marc ALBERT
# DATE: 2026-04-16
# VERSION: 1.0
#
# OBJECTIVE:
#   Inventory VSCodium extensions from user profiles and export a JSON file that
...
function collect_vscodium_extensions_for_product {
    ...
    for user_dir in "${user_dirs[@]}"; do
        ...
        while IFS= read -r ext_dir; do
            ...
            record+="\"ExtensionPlatform\":\"VSCodium\","
...
if test_is_remote_ops; then
    OUTPUT_FILE="$S1_OUTPUT_DIR_PATH/VSCodium_Extensions_Inventory.json"
    LOG_FILE="$S1_OUTPUT_DIR_PATH/VSCodium_Extensions_Inventory.log"
else
    OUTPUT_FILE="${TMPDIR:-/tmp}/VSCodium_Extensions_Inventory_${timestamp}.json"
    LOG_FILE="${TMPDIR:-/tmp}/VSCodium_Extensions_Inventory_${timestamp}.log"
fi
...
write_log "Starting VSCodium extension inventory..."
...
# VSCodium stores extensions in ~/.vscode-oss/extensions
collect_vscodium_extensions_for_product "VSCodium" ".vscode-oss/extensions" "${USER_DIRS[@]}"
...
if [[ $extension_count -gt 0 ]]; then
    ...
    write_log "No VSCodium extensions found. Metadata-only JSON saved at: $OUTPUT_FILE" "INFO"
fi
