...
- **Windsurf** (Windows: `.windsurf\extensions`, macOS: `~/Library/Application Support/Windsurf/User/extensions` and `~/.windsurf/extensions`)
...
- Supports Windsurf extension location
...
- Description
- Categories
- VS Code engine compatibility (`engines.vscode` — Windsurf uses VS Code–compatible extensions)
...
- **RemoteOps**: `$Env:S1_OUTPUT_DIR_PATH` (Windows) / `$S1_OUTPUT_DIR_PATH` (macOS)
  - `Windsurf_Extensions_Inventory.json`
  - `Windsurf_Extensions_Inventory.log`
- **Local execution**: `$Env:TEMP` (Windows) / `$TMPDIR` (macOS)
  - `Windsurf_Extensions_Inventory_<timestamp>.json`
  - `Windsurf_Extensions_Inventory_<timestamp>.log`
...
- `RecordType`
- `ExtensionPlatform` (always "Windsurf")
- `ExtensionProduct`
- `ExtensionUser`
- `ExtensionID`
- `ExtensionName`
...
[
  ...
  },
  {
    ...
    "ExtensionPlatform": "Windsurf",
    "ExtensionProduct": "Windsurf",
    "ExtensionUser": "jdoe",
    ...
    "ExtensionInstallPath": "C:\\Users\\jdoe\\.windsurf\\extensions\\ms-python.python-2026.2.0",
    "ExtensionManifestPath": "C:\\Users\\jdoe\\.windsurf\\extensions\\ms-python.python-2026.2.0\\package.json",
...
```powershell
powershell -ExecutionPolicy Bypass -File ".\collect_windsurf_extensions.ps1"
```

Or from the script directory:

```powershell
.\collect_windsurf_extensions.ps1
```

### macOS (Bash)

```bash
chmod +x collect_windsurf_extensions.sh
./collect_windsurf_extensions.sh
```
...
- Windsurf is a VS Code fork and uses the same extension format, so extensions have `engines.vscode` in their manifest.
