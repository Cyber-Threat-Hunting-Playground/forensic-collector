...
<#
.SYNOPSIS
    Collect installed Windsurf extensions from Windows user profiles.
...
.EXAMPLE
    ...
    Collects Windsurf extension inventory and writes JSON and log files.
...
function Get-WindsurfExtensions {
    param (
        [Parameter(Mandatory = $true)]
        [array]$UserProfiles,
        [Parameter(Mandatory = $true)]
...
if (Test-IsRemoteOps) {
    $outputFile = Join-Path $Env:S1_OUTPUT_DIR_PATH "Windsurf_Extensions_Inventory.json"
    $script:LogFilePath = Join-Path $Env:S1_OUTPUT_DIR_PATH "Windsurf_Extensions_Inventory.log"
}
else {
    $outputFile = Join-Path $Env:TEMP "Windsurf_Extensions_Inventory_$timestamp.json"
    $script:LogFilePath = Join-Path $Env:TEMP "Windsurf_Extensions_Inventory_$timestamp.log"
}

Write-Log -Message "Starting Windsurf extension inventory..."
...
# Windsurf stores extensions in .windsurf\extensions (at the root of each user profile)
$allExtensions = @()
$allExtensions += Get-WindsurfExtensions -UserProfiles $userProfiles -ProductName "Windsurf" -RelativeExtensionsPath ".windsurf\extensions"
...
if ($allExtensions.Count -gt 0) {
    ...
    Write-Log -Message "No Windsurf extensions found. Metadata-only JSON saved at: $outputFile" -IsInfo
}
