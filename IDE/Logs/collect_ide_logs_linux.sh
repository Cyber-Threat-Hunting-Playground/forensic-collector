...
# ==============================================================================
# SCRIPT: collect_ide_logs_linux.sh
...
#
# BUILT-IN TOOLS ONLY (no Python, jq, or pip):
#   bash, grep, sed, stat, date, hostname, uname, find, tr
# ==============================================================================
...
function collect_user_dirs {
    ...
    # Also check /root if running as root
...
function parse_sharedprocess_log {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ ^([0-9]{4}-[0-9]{2}-[0-9]{2}[[:space:]]+[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)[[:space:]]+\[([a-zA-Z]+)\][[:space:]]+Installing\ extension:[[:space:]]+([^[:space:]]+)[[:space:]]+(\{.+\})$ ]]; then
            ts="${BASH_REMATCH[1]}"; lvl="${BASH_REMATCH[2]}"; ext_id="${BASH_REMATCH[3]}"; json_payload="${BASH_REMATCH[4]}"
...
if test_is_remote_ops; then
    ...
    OUTPUT_FILE="/tmp/IDE_Logs_Inventory_${timestamp}.json"
    LOG_FILE="/tmp/IDE_Logs_Inventory_${timestamp}.log"
else
    OUTPUT_FILE="/tmp/IDE_Logs_Inventory.json"
    LOG_FILE="/tmp/IDE_Logs_Inventory.log"
fi
...
# Linux log paths live under ~/.config/<product>/logs/
declare -a PRODUCTS=(
    "Visual Studio Code|.config/Code/logs"
    "Visual Studio Code Insiders|.config/Code - Insiders/logs"
    "VSCodium|.config/VSCodium/logs"
