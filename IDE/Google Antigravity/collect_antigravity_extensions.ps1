...
<#
.SYNOPSIS
    Collect installed Google Antigravity extensions from Windows user profiles.
...
.EXAMPLE
    ...
    Collects Google Antigravity extension inventory and writes JSON and log files.

.NOTES
    Author: Replicated from Cursor Extensions (Jean-Marc ALBERT)
    Date: 2026-04-16
    Version: 1.0
#>
...
if (Test-IsRemoteOps) {
    $outputFile = Join-Path $Env:S1_OUTPUT_DIR_PATH "Antigravity_Extensions_Inventory.json"
    $script:LogFilePath = Join-Path $Env:S1_OUTPUT_DIR_PATH "Antigravity_Extensions_Inventory.log"
}
else {
    $outputFile = Join-Path $Env:TEMP "Antigravity_Extensions_Inventory_$timestamp.json"
    $script:LogFilePath = Join-Path $Env:TEMP "Antigravity_Extensions_Inventory_$timestamp.log"
}

Write-Log -Message "Starting Google Antigravity extension inventory..."
...
# Google Antigravity stores extensions in .antigravity\extensions (at the root of each user profile)
$allExtensions = @()
$allExtensions += Get-AntigravityExtensions -UserProfiles $userProfiles -ProductName "Antigravity" -RelativeExtensionsPath ".antigravity\extensions"
...
if ($allExtensions.Count -gt 0) {
    ...
    Write-Log -Message "No Google Antigravity extensions found. Metadata-only JSON saved at: $outputFile" -IsInfo
}
