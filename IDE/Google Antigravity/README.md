# Google Antigravity Extension Inventory
...
- **Google Antigravity** (Windows: `.antigravity\extensions`, macOS: `~/Library/Application Support/Antigravity/User/extensions` and `~/.antigravity/extensions`)
...
- Supports Google Antigravity extension location
...
- Description
- Categories
- VS Code engine compatibility (`engines.vscode` — Google Antigravity uses VS Code–compatible extensions)
...
- **RemoteOps**: `$Env:S1_OUTPUT_DIR_PATH` (Windows) / `$S1_OUTPUT_DIR_PATH` (macOS)
  - `Antigravity_Extensions_Inventory.json`
  - `Antigravity_Extensions_Inventory.log`
- **Local execution**: `$Env:TEMP` (Windows) / `$TMPDIR` (macOS)
  - `Antigravity_Extensions_Inventory_<timestamp>.json`
  - `Antigravity_Extensions_Inventory_<timestamp>.log`
...
- `RecordType`
- `ExtensionPlatform` (always "Google Antigravity")
- `ExtensionProduct`
- `ExtensionUser`
- `ExtensionID`
- `ExtensionName`
...
[
  ...
  },
  {
    ...
    "ExtensionPlatform": "Google Antigravity",
    "ExtensionProduct": "Antigravity",
    "ExtensionUser": "jdoe",
    ...
    "ExtensionInstallPath": "C:\\Users\\jdoe\\.antigravity\\extensions\\ms-python.python-2026.2.0",
    "ExtensionManifestPath": "C:\\Users\\jdoe\\.antigravity\\extensions\\ms-python.python-2026.2.0\\package.json",
...
```powershell
powershell -ExecutionPolicy Bypass -File ".\collect_antigravity_extensions.ps1"
```

Or from the script directory:

```powershell
.\collect_antigravity_extensions.ps1
```

### macOS (Bash)

```bash
chmod +x collect_antigravity_extensions.sh
./collect_antigravity_extensions.sh
```
...
- Google Antigravity is a VS Code fork and uses the same extension format, so extensions have `engines.vscode` in their manifest.
