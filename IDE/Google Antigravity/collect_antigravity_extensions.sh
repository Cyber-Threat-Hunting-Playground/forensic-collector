...
# ==============================================================================
# SCRIPT: collect_antigravity_extensions.sh
# DESCRIPTION: Collect installed Google Antigravity extensions on macOS.
# AUTHOR: Jean-Marc ALBERT
# DATE: 2026-04-16
# VERSION: 1.0
#
# OBJECTIVE:
#   Inventory Google Antigravity extensions from user profiles and export a JSON
#   file that matches the Windows collector schema.
#
...
function collect_antigravity_extensions_for_product {
    ...
    for user_dir in "${user_dirs[@]}"; do
        ...
        while IFS= read -r ext_dir; do
            ...
            record+="\"ExtensionPlatform\":\"Google Antigravity\","
...
if test_is_remote_ops; then
    OUTPUT_FILE="$S1_OUTPUT_DIR_PATH/Antigravity_Extensions_Inventory.json"
    LOG_FILE="$S1_OUTPUT_DIR_PATH/Antigravity_Extensions_Inventory.log"
else
    OUTPUT_FILE="${TMPDIR:-/tmp}/Antigravity_Extensions_Inventory_${timestamp}.json"
    LOG_FILE="${TMPDIR:-/tmp}/Antigravity_Extensions_Inventory_${timestamp}.log"
fi
...
write_log "Starting Google Antigravity extension inventory..."
...
# Google Antigravity may store extensions in either location depending on version/config:
# - ~/Library/Application Support/Antigravity/User/extensions (standard Electron app path)
# - ~/.antigravity/extensions (alternative path used by some versions)
collect_antigravity_extensions_for_product "Antigravity" "Library/Application Support/Antigravity/User/extensions" "${USER_DIRS[@]}"
collect_antigravity_extensions_for_product "Antigravity" ".antigravity/extensions" "${USER_DIRS[@]}"
...
if [[ $extension_count -gt 0 ]]; then
    ...
    write_log "No Google Antigravity extensions found. Metadata-only JSON saved at: $OUTPUT_FILE" "INFO"
fi
