...
<#
.SYNOPSIS
    Collect installed VSCodium extensions from Windows user profiles.
...
.EXAMPLE
    ...
    Collects VSCodium extension inventory and writes JSON and log files.
...
function Get-VSCodiumExtensions {
    param (
        [Parameter(Mandatory = $true)]
        [array]$UserProfiles,
        [Parameter(Mandatory = $true)]
...
if (Test-IsRemoteOps) {
    $outputFile = Join-Path $Env:S1_OUTPUT_DIR_PATH "VSCodium_Extensions_Inventory.json"
    $script:LogFilePath = Join-Path $Env:S1_OUTPUT_DIR_PATH "VSCodium_Extensions_Inventory.log"
}
else {
    $outputFile = Join-Path $Env:TEMP "VSCodium_Extensions_Inventory_$timestamp.json"
    $script:LogFilePath = Join-Path $Env:TEMP "VSCodium_Extensions_Inventory_$timestamp.log"
}

Write-Log -Message "Starting VSCodium extension inventory..."
...
# VSCodium stores extensions in .vscode-oss\extensions (at the root of each user profile)
$allExtensions = @()
$allExtensions += Get-VSCodiumExtensions -UserProfiles $userProfiles -ProductName "VSCodium" -RelativeExtensionsPath ".vscode-oss\extensions"
...
if ($allExtensions.Count -gt 0) {
    ...
    Write-Log -Message "No VSCodium extensions found. Metadata-only JSON saved at: $outputFile" -IsInfo
}
